...
// Forward declaration for comparison benchmarks
...
class BenchmarkTimer
{
  public:
    ...
    static double benchmark(const std::string& name, Func&& func, size_t iterations = DEFAULT_ITERATIONS)
    {
        ...
        std::cout << std::left << std::setw(35) << name << std::right << std::setw(10) << std::fixed
                  << std::setprecision(2) << avg_ns << " ns" << std::endl;
...
class RandomGenerator
{
  ...
  public:
    ...
    lina::vec3<T> random_vec3() const { return { random(), random(), random() }; }

    lina::mat<T, 2, 2> random_mat2() const { return { random(), random(), random(), random() }; }
...
void benchmark_vector_operations()
{
    ...
    // Vector norm (length)
    BenchmarkTimer::benchmark_with_result("Vector norm", [&]() { return lina::norm(v1); });

    // Vector norm squared (more efficient)
    BenchmarkTimer::benchmark_with_result("Vector norm squared", [&]() { return lina::norm2(v1); });

    // Vector normalization
    BenchmarkTimer::benchmark_with_result("Vector normalize", [&]() { return lina::normalize(v1); });

    // Dot product
    BenchmarkTimer::benchmark_with_result("Vector dot product", [&]() { return lina::dot(v1, v2); });

    // Cross product
    BenchmarkTimer::benchmark_with_result("Vector cross product", [&]() { return lina::cross(v1, v2); });

    // Vector distance
    BenchmarkTimer::benchmark_with_result("Vector distance", [&]() { return lina::distance(v1, v2); });

    // Vector-scalar operations
    BenchmarkTimer::benchmark_with_result("Vector * scalar", [&]() { return v1 * static_cast<T>(2.5); });

    BenchmarkTimer::benchmark_with_result("Vector + Vector", [&]() { return v1 + v2; });
}
...
void benchmark_matrix_operations()
{
    ...
    // 2x2 Matrix operations
    {
        ...
        BenchmarkTimer::benchmark_with_result("Matrix 2x2 multiply", [&]() { return m1 * m2; });

        BenchmarkTimer::benchmark_with_result("Matrix 2x2 transpose", [&]() { return lina::transpose(m1); });

        BenchmarkTimer::benchmark_with_result("Matrix 2x2 determinant", [&]() { return lina::det(m1); });

        const auto inv_m = rng.random_invertible_mat3(); // Use 3x3 for invertible
        BenchmarkTimer::benchmark_with_result("Matrix 2x2 inverse (constexpr)", [&]() { return lina::c_inverse(m1); });
    }
    ...
    // 3x3 Matrix operations
    {
        ...
        BenchmarkTimer::benchmark_with_result("Matrix 3x3 multiply", [&]() { return m1 * m2; });

        BenchmarkTimer::benchmark_with_result("Matrix 3x3 * Vector", [&]() { return m1 * v; });

        BenchmarkTimer::benchmark_with_result("Matrix 3x3 transpose", [&]() { return lina::transpose(m1); });

        BenchmarkTimer::benchmark_with_result("Matrix 3x3 determinant", [&]() { return lina::det(m1); });
        ...
        BenchmarkTimer::benchmark_with_result("Matrix 3x3 inverse (runtime)", [&]() { return lina::inverse(inv_m); });
    }
    ...
    // 4x4 Matrix operations
    {
        ...
        BenchmarkTimer::benchmark_with_result("Matrix 4x4 multiply", [&]() { return m1 * m2; });

        BenchmarkTimer::benchmark_with_result("Matrix 4x4 * Vector (homogeneous)", [&]() { return m1 * v; });

        BenchmarkTimer::benchmark_with_result("Matrix 4x4 transpose", [&]() { return lina::transpose(m1); });

        BenchmarkTimer::benchmark_with_result("Matrix 4x4 determinant", [&]() { return lina::det(m1); });

        const auto inv_m = rng.random_invertible_mat4();
        BenchmarkTimer::benchmark_with_result("Matrix 4x4 inverse (constexpr)",
                                              [&]() { return lina::c_inverse(inv_m); });

        BenchmarkTimer::benchmark_with_result("Matrix 4x4 inverse (runtime)", [&]() { return lina::inverse(inv_m); });
    }
}
...
void benchmark_constexpr_math()
{
    ...
    BenchmarkTimer::benchmark_with_result("sin(π/6)", [&]() { return lina::sin(angle); });

    BenchmarkTimer::benchmark_with_result("cos(π/6)", [&]() { return lina::cos(angle); });

    BenchmarkTimer::benchmark_with_result("tan(π/6)", [&]() { return lina::tan(angle); });

    BenchmarkTimer::benchmark_with_result("sqrt(2.5)", [&]() { return lina::sqrt(value); });

    BenchmarkTimer::benchmark_with_result("abs(-2.5)", [&]() { return lina::abs(-value); });
}
...
void benchmark_transformations()
{
    ...
    BenchmarkTimer::benchmark_with_result("Identity matrix 4x4", [&]() { return lina::identity<T, 4>(); });

    BenchmarkTimer::benchmark_with_result("Translation matrix", [&]() { return lina::translation(vec); });

    BenchmarkTimer::benchmark_with_result("Rotation X matrix", [&]() { return lina::rotation_x(angle); });

    BenchmarkTimer::benchmark_with_result("Rotation Y matrix", [&]() { return lina::rotation_y(angle); });

    BenchmarkTimer::benchmark_with_result("Rotation Z matrix", [&]() { return lina::rotation_z(angle); });

    BenchmarkTimer::benchmark_with_result("Rotation arbitrary axis",
                                          [&]() { return lina::rotation(lina::normalize(vec), angle); });

    BenchmarkTimer::benchmark_with_result("Scale matrix", [&]() { return lina::scale(vec); });

    const auto eye    = rng.random_vec3();
    const auto center = rng.random_vec3();
    const auto up     = lina::vec3<T>{ 0, 1, 0 };

    BenchmarkTimer::benchmark_with_result("Look-at matrix", [&]() { return lina::look_at(eye, center, up); });

    BenchmarkTimer::benchmark_with_result("Perspective projection", [&]() {
        return lina::perspective(
            lina::pi<T> / 4, static_cast<T>(16.0 / 9.0), static_cast<T>(0.1), static_cast<T>(100.0));
    });

    BenchmarkTimer::benchmark_with_result("Orthographic projection", [&]() {
        return lina::ortho(static_cast<T>(-10),
                           static_cast<T>(10),
                           static_cast<T>(-10),
                           static_cast<T>(10),
                           static_cast<T>(0.1),
...
void benchmark_comparisons()
{
    ...
    BenchmarkTimer::benchmark_with_result("almost_equal (scalars)", [&]() { return lina::almost_equal(v1.x, v2.x); });

    BenchmarkTimer::benchmark_with_result("almost_equal (vectors)", [&]() { return lina::almost_equal(v1, v2); });

    BenchmarkTimer::benchmark_with_result("almost_equal (matrices)", [&]() { return lina::almost_equal(m1, m2); });
...
void benchmark_compile_time()
{
    ...
    BenchmarkTimer::benchmark_with_result("Access compile-time matrix", [&]() { return ct_mvp(0, 0); });

    BenchmarkTimer::benchmark_with_result("Access compile-time sin(π/6)", [&]() { return ct_sin; });

    BenchmarkTimer::benchmark_with_result("Access compile-time sqrt(2)", [&]() { return ct_sqrt; });

    BenchmarkTimer::benchmark_with_result("Access compile-time normalized", [&]() { return ct_normalized.x; });
    ...
    std::cout << "  normalize({3,4,0}) = {" << ct_normalized.x << ", " << ct_normalized.y << ", " << ct_normalized.z
              << "}" << std::endl;
}
...
void benchmark_memory_access()
{
    ...
    // Sequential access
    BenchmarkTimer::benchmark_with_result(
        "Sequential matrix access",
        [&]() {
            T sum = 0;
